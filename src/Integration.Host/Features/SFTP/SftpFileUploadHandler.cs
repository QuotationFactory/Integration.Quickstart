...
public class SftpFileUploadHandler : INotificationHandler<OutputFileCreated>
{
    ...
    public async Task Handle(OutputFileCreated notification, CancellationToken cancellationToken)
    {
        ...
        finally
        {
            if (processedSuccess)
            {
                ...
                _logger.LogInformation("Moved file {filePath} to processed directory {processedDirectory}",notification.FilePath, result);
                ...
                if (_sftpSettings.UploadZipFile)
                {
                    ...
                    _logger.LogInformation("Moved file {zipFilePath} to processed directory {processedDirectoryZip}", zipFilePath, resultZip);
                }
            }
            else
            {
    ...
    private async Task UploadFileToSftpAsync(string filePath, CancellationToken cancellationToken)
    {
        try
        {
            ...
            _logger.LogInformation("Uploading file {filePath} to SFTP server {server} with user {user}", filePath, _sftpSettings.Host, _sftpSettings.Username);
            ...
            _logger.LogInformation("File {filePath} uploaded to SFTP server {server} with user {user}", filePath, _sftpSettings.Host, _sftpSettings.Username);
